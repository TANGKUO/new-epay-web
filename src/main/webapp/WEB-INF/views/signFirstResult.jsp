...
<input type=button onclick="window.open('<%=url %>/epay/query.action','_parent')"  value="查证" style="font: normal 16px arial, sans-serif;padding: 5px 30px 5px 30px;margin : 18px;" />
	...
	<p>响应头</p>
		...
		<c:choose>
			...
			<p>成功响应体</p>
				<c:choose>
				<c:when test="${result['TransId'] eq 'NUPRes'}">
					<table style="width:670px;background-color: #93FF93; border-width: 2px; margin: auto; padding: 20px;" border="1">
								<tr>
									<td  style="width:37%"><font color="red">*</font>手机号码</td>
									<td  style="width:27%">${result['MobNo']}</td>
									<td  style="width:28%"></td>
									<td>11</td>
								</tr>
								<tr>
									<td><font color="red">*</font>是否发送验证码</td>
									<td>${result['Status']}</td>
									<td>0- 发送失败 1-发送成功</td>
									<td>1</td>
								</tr>
				...
				<c:otherwise>
					<table style="width:670px;background-color: #93FF93; border-width: 2px; margin: auto; padding: 20px;" border="1">
							<tr>
								<td  style="width:37%"><font color="red">*</font>签约协议号</td>
								<td  style="width:27%">${result['SignNo']}</td>
								<td  style="width:28%"></td>
								<td>34</td>
							</tr>
							<tr>
								<td><font color="red">*</font>银行账户类型</td>
								<td>${result['BkAcctTp']}</td>
								<td>0 – 银行借记账户；<br/>
									1 – 银行贷记账户；<br/>
									2 – 银行准贷记账户</td>
								<td>1</td>
							...
							<tr>
								<td><font color="red">*</font>银行账户等级</td>
								<td>${result['BkAcctLvl']}</td>
								<td>1 – I类账户；<br/>
									2 – II类账户；<br/>
									3 – III类账户</td>
								<td>1</td>
		...
		<input type=button onclick="window.open('<%=url %>/epay/signSecond.action')"  value="下一步" style="font: normal 16px arial, sans-serif;padding: 5px 30px 5px 30px;margin : 18px;" />
